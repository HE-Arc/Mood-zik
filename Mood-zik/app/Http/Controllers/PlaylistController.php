...
class PlaylistController extends Controller
{
    ...
    public function showPlaylist()
    {
      ...
      $user_id = auth()->id();
      $playlist = DB::table('posts')
          ->join('users', 'users.id', '=', 'posts.user_id')->select('posts.*', 'users.name')
          ->join('post_playlist', 'users.name', '=', 'post_playlist.username')
          ->where('posts.id', '=', $user_id)
          ->get();
      //$playlist = DB::table('posts')->where('post_playlist.playlist_id', '=', $user_id)->distinct('posts.*')->get();

      $usernames = DB::table('post_playlist')->where('playlist_id', '=', $user_id)->pluck('username');
